/* script.js — Homework 8
   Discrete approximation of Poisson counting process and plotting.
   Place this file in the same folder as homework8.html and load via <script src="script.js"></script>.
*/
...
(() => {
  ...
  function sampleBinomial(n, prob) {
    // simple binomial by summing Bernoulli; n may be large but prob small -> ok for our discrete approx
    let cnt = 0;
    for (let i = 0; i < n; i++) if (Math.random() < prob) cnt++;
    return cnt;
  }

  // produce event times from discrete binomial modeling (if desired)
  function discreteEventTimes(n, T, prob) {
    const dt = T / n;
    const times = [];
    for (let i = 0; i < n; i++) {
      if (Math.random() < prob) {
        // place event uniformly inside the subinterval for a slightly better approximation
  ...
  function runSimulation() {
    ...
    const prob = (lambda * T) / n; // success prob per subinterval
    // precaution: if prob > 1, binomial model invalid; warn
    if (prob > 1) { alert('λT / n must be ≤ 1. Increase n or decrease λT.'); return; }

    const counts = new Map();
    const samplePaths = [];
    const allEventTimes = []; // collect event times for first sample if needed
    ...
    for (let r = 0; r < runs; r++) {
      ...
      if (r < showPath) {
        ...
        if (r === 0) allEventTimes.push(...times);
      }
    }
    ...
    // theoretical pmf scaled by runs
    const lambdaT = lambda * T;
    const theo = keys.map(k => poissonPMF(k, lambdaT) * runs);
    histChart.data.datasets[1].data = theo;
    histChart.update();

    // prepare path chart: cumulative step functions for each sample path
    const datasets = samplePaths.map((times, idx) => {
      // create stepwise cumulative counts at a regular time grid (for smooth lines)
      const gridN = 200;
    ...
    // add expected line (deterministic trend λ t)
    const gridN = 200;
    ...
    // feed chart with first dataset as expected line, others as samples
    ...
    // put code in the viewer (if not already)
    ...
    // console info
    console.log('Simulation done. Empirical mean count ≈', weightedMean(keys, freqs), 'theoretical λT =', lambdaT);
  }
